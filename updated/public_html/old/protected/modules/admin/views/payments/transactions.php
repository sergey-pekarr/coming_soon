...
?>
    ...
    <dd>
        <?php echo $form->dropDownList($model,'paymod', CHelperPayment::getPaymodsForSelect() /*$model->paymodsForSelect*/); ?>
	    <?php /*if ($model->paymod=='netbilling_2_cams_sub') { ?>
	        <br />
	        <?php echo $form->dropDownList($model,'type_3', array(''=>'All', 'Presale'=>'Presale', 'Spender'=>'Spender'), array('title'=>'Type 3 for netbilling_2_cams_sub') ); ?>
	    <?php }*/ ?>
    </dd>
    ...
    <dd>
        <?php echo $form->dropDownList($model,'status', array(''=>"All", 'started'=>"started",'authed'=>"authed",'completed'=>"completed",'completed_cams'=>"completed_cams",'renewal'=>"renewal",'renewal_declined'=>"renewal_declined",'cancelled'=>"cancelled")); ?>
    </dd> 
    
    <dt><?php echo $form->labelEx($model,'form'); ?></dt>
    <dd>
        <?php echo $form->dropDownList($model,'form', CHelperAdmin::getForms(true)/*array(''=>'All','1'=>'1 - main form','88'=>'88 - api','93'=>'93 - zsignup','932'=>'932 - zsignup2')*/); ?>
    </dd>    
...
<thead>
    <tr>
	    ...
	    <th>User</th>
	    <th>Aff id <br /><br /> Manager name</th>
	    <th>Form</th>
	    <th>Amount <br /><br /> <span class="smaller">Price ID</span> </th>	    
...
//FB::warn($rows, 'rows');  	
  	if ($rows['list']) 
  		foreach ($rows['list'] as $row) { 
  			...
  			$firstPay = (isset($paymentInfo['firstpay'])) ? $paymentInfo['firstpay'] : "";	
  		?>
		<tr class="tr_transaction_<?php echo $row['status'] ?>">
	   		...
	   		<td>
	   			<?php echo $row['paymod'] ?>
	   			<?php if (isset($row['nb_processor']) && $row['nb_processor']) { ?>
	   				<br /><br />
	   				<span class="smaller"><?php echo $row['nb_processor'] ?></span>
	   			<?php } ?>
	   		</td>
	   		...
	   		<td title="Added: <?php echo $row['added'] ?>, Updated: <?php echo $row['updated'] ?>">
	   			...
	   			<?php if (isset($row['renewalInitialInfo'])) { ?>
	   				<div class="smallerInfo">
	   					Firstpay at: <br /> <?php echo $firstPay;//$row['renewalInitialInfo'] ?>
	   				</div>
	   			<?php } ?>
	   		</td>
	   		...
	   		<td>
	   			<?php echo $profile->getDataValue('affid') ?>
	   			
	   			<?php $manager = HelperAff::getManagerOfAff($profile->getDataValue('affid'))?>
	   			<?php if ($manager) { ?>
		   			<br /><br />
		   			<span title="manager name"><?php echo $manager['login']?></span>
	   			<?php } ?>
	   		</td>
	   		...
	   		<td>
	   			$<?php echo $row['amount'] ?>
	   			<br /><br />
	   			<span class="smaller"><?php echo $row['price_id'] ?></span>
	   		</td>
