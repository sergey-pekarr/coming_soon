...
class Oneclick 
{
	...
	public static function EncodeURLParameters( $email, $password, $authkey, $makeassoc = false, $urlencode = true )
	{
		$result = array ( 'chksm' => '', 'email'  => '', 'pwd' => '', 'chksmv' => '' );
		$email = strtolower($email);
		$ver = '1.5';
	
		$result['chksm'] = self::OneclickMD5Checksum( $authkey, $email, $password );
		$result['email'] = self::OneclickEncode( $email, true, false, $ver );
		$result['pwd']   = self::OneclickEncode( $password, true, false, $ver );
		$result['chksmv']  = $ver;
	...
	public static function OneclickEncode( $str, $encode = true, $mday = false, $ver = 0 )
	{
	  if( $mday === false )
	  ...
	  for( $i = 0 ; $i < strlen( $str ) ; $i++ )
	  {
	  	...
	  	$orgval = self::translateChar( $str{$i}, false, $ver );
	    ...
	    if ( ($newval = self::translateChar( ($orgval+$shift), true, $ver )) !== false )
	    	$x .= $newval;
	    else $x .= $str{$i};
	  }
	...
	public static function OneclickMD5Checksum( $authkey, $email, $pass, $timekey = false )
	{
	  if( $timekey === false )
	...
	public static function translateChar( $c, $tostr, $ver )
	{
		$enclist = array (
