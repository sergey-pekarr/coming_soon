<?php
class Payment_zombaio2 extends Payment
{
	private $pmName = 'zombaio2'; 
	
	private $z_id = 0;//zom internal transaction id
	
	public $availablePriceIds = array(2);
	public $defaultPriceId = 2;
	
	private $allin9Mode = true;//using for /allin9 or /payment
	
	private $zom_SiteId = "xxx";
	private $zom_GWPass = "xxx";
	
	
	public function __construct($id=0, $allin9Mode=true)
	{
		parent::__construct($id, $this->pmName);
		
		$this->allin9Mode = $allin9Mode;
	} 
	...
	public function startTransaction($user_id, $price_id)
	{
		...
		$return_url = SITE_URL;
		if ($this->allin9Mode)
			$return_url_decline = urlencode( SITE_URL."/allin9/step4" );
		else
		...
		CHelperLog::logFile('pm_zombaio2_submit.log', $url);		
	...
	public function postback($g)
	{
		...
		CHelperLog::logFile('pm_zombaio2_postback.log', $s);	
