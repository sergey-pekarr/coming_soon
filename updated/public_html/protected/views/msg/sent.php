...
<div style="padding-top: 10px; margin-bottom: 20px;">
    <p>
        ...
        you wish to view.</p><br>
		<p><strong>
		N.B. Only messages sent with no reply will appear in your sent items.</strong></p>
    <ul class="message-list">
<?php foreach($items as $item) { 
	if(!$item['id_'.$dir]) continue;
    $id = $item['id_'.$dir];
    $profile = new Profile($id);
	$msgid = Yii::app()->secur->encryptID($id).'/'.((isset($item['parent']) && $item['parent']!= '')?$item['parent']:$item['id']);
	$read = ($item['read'] == '1');
	$itemClass = $read?'read':'unread';
?>
		<li class="spacer"></li>
		<li class="message-row">
            <table id="<?php echo $msgid ?>" class="messagerow <?php echo $itemClass; ?>">
                <tbody>
                    <tr>
                        ...
                        <td class="actionsread" width="10">
                            <a onclick="" href="/thread/<?php echo $msgid; ?>">
