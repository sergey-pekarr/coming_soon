...
//AdminDbHttpSession class for admin area
//based on CDbHttpSession (fixed utf8 in session table + added IP control)
 ...
 * @since 1.0
 */
class AdminDbHttpSession extends CHttpSession
{
	/**
...
WHERE expire>$now AND id=:id AND ip=:ip
";
		$data=$this->getDbConnection()->createCommand($sql)->bindValue(':id',$id)->bindValue(':ip', CHelperLocation::getIPReal() )->queryScalar();
	...
	public function writeSession($id,$data)
	{
		...
		try
		{
			...
			{
				$sql="INSERT INTO {$this->sessionTableName} (id, data, expire, ip) VALUES (:id, :data, $expire, :ip)";
				$db->createCommand($sql)
					->bindValue(':id',$id)
					->bindValue(':data',$data)
					->bindValue(':ip',CHelperLocation::getIPReal())
					->execute();
			}
			else
			{
